...
class QwarpPlusMinus(shell.Task["QwarpPlusMinus.Outputs"]):
    """
    Examples
    -------
    ...
    >>> task.inputs.in_file = NiftiGz.mock("sub-01_dir-LR_epi.nii.gz")
    ...
    out_file: Path = shell.arg(
        ...
        position=1,
        default="Qwarp.nii.gz",
    )
    plusminus: bool = shell.arg(
        help="Normally, the warp displacements dis(x) are defined to matchbase(x) to source(x+dis(x)).  With this option, the matchis between base(x-dis(x)) and source(x+dis(x)) -- the twoimages 'meet in the middle'. For more info, view Qwarp` interface",
        argstr="-plusminus",
        position=2,
        default=True,
    )
