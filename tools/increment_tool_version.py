...
TASKS_DIR = PKG_DIR / "pydra" / "tasks" / "afni"
VERSION_GRANULARITY = (
    2  # Number of version parts to include: 1 - major, 2 - minor, 3 - micro
)

# Ensure that current package is on the path
...
def increment_tool_version(new_version: LooseVersion):
    ...
    if not sub_pkg_dir.exists():
        ...
        prev_ver_mod = import_module(f"pydra.tasks.afni.{prev_version}")
        ...
        code_str = (
            f"from pydra.tasks.afni import {prev_version}\n" "import . _tool_version\n"
        )
