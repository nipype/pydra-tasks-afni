...
class AFNIConverter:
    ...
    def __init__(self, interface_name, interface_spec_file):
        ...
        nipype_interface = getattr(afni, self.interface_name)
...
AFNI_MODULES = ['base', 'model', 'preprocess', 'svm', 'utils']
...
@click.option(
    "-m", "--module_name", required=True, help=f"name of the module from the list {AFNI_MODULES}"
)
def create_pydra_spec(interface_name, module_name):
    if module_name not in AFNI_MODULES:
        raise Exception(
            f"module name {module_name} not available;" f"should be from the list {AFNI_MODULES}"
        )

    spec_file = Path(os.path.dirname(__file__)) / f"../specs/afni_{module_name}_param.yml"
    ...
    def all_interfaces(module):
        nipype_module = getattr(afni, module)
    ...
    dirname_interf = Path(__file__).parent.parent / f"pydra/tasks/afni/{module_name}"
    dirname_interf.mkdir(exist_ok=True)

    for interface_el in interface_list:
        converter = AFNIConverter(
            interface_name=interface_el,
            interface_spec_file=Path(__file__).parent.parent
            / f"specs/afni_{module_name}_param.yml",
        )
        converter.pydra_specs(write=False, dirname=dirname_interf)
